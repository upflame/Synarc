...
.DESCRIPTION
  ...
  has all 4 tier sections (Tier 1 â€” Native Execution, Tier 2 â€” External
  Integration, Tier 3 â€” Manual Workflow, Tier 4 â€” Human-Assisted).
...
function Get-CapabilityNames {
    ...
    foreach ($line in $lines) {
        ...
        if ($trimmed -match '^###\s+(?:Capability:\s*)?(.+?)(?:\s*â€”\s*|\s*â€“\s*|\s*-\s*|\s*$)' -or
...
foreach ($dir in $skillDirs) {
    ...
    if (-not (Test-Path -LiteralPath $skillMdPath)) {
        Write-Host "  [SKIP] $skillName â€” SKILL.md not found" -ForegroundColor DarkGray
        continue
    }
    ...
    try {
        ...
        Write-Host "  [ERR]  $skillName â€” Cannot read SKILL.md" -ForegroundColor Red
        continue
    }
    ...
    if ($missingTiers.Count -gt 0) {
        Write-Host "  [FAIL] $skillName â€” $($missingTiers.Count) incomplete capabilities" -ForegroundColor Red
        if ($Verbose) {
        ...
        Write-Host "  [INFO] $skillName â€” No capability blocks detected" -ForegroundColor DarkYellow
    } else {
        Write-Host "  [PASS] $skillName â€” $skillCapCount capabilities, all 4 tiers present" -ForegroundColor Green
    }
...
foreach ($r in $skillResults) {
    Write-Host "$($r.Status) $($r.Skill) â€” $($r.Complete)/$($r.CapCount) complete"
