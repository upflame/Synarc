...
foreach ($dir in $skillDirs) {
    ...
    $capSections = @()
    $capPattern = [regex]::Matches($content, '(?m)^#{1,6}\s+(.+?)(?:\s*—\s*|\s*–\s*|\s*-\s*)?(?:Capability.*)?$')
    ...
    # Strip code blocks (fenced ``` and inline `) before matching to avoid
    # false positives on Python function calls like obj[i](x, y).
    # Use a fence opener followed by a newline so we don't match empty
    # adjacent fence pairs ` ``` ` + ` ``` ` which the non-greedy regex
    # would otherwise prefer over the actual content blocks.
    $contentNoCode = [regex]::Replace($content, '(?ms)^```[^\r\n]*\r?\n.*?\r?\n^```\s*$', '', 'Multiline')
    $contentNoCode = [regex]::Replace($contentNoCode, '`[^`\r\n]+`', '')
    $refs = [regex]::Matches($contentNoCode, '\[([^\]]+)\]\(([^)]+)\)')
    $resolvedCount = 0
    foreach ($ref in $refs) {
