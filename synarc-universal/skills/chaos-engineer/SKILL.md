...
def inject_latency(experiment_id, parameters=None):
    """Inject network latency using tc (traffic control) or Toxiproxy."""
    ...
    try:
        ...
        # Using tc to add netem delay on Linux
        subprocess.run(
            ["tc", "qdisc", "add", "dev", interface, "root", "netem", "delay", f"{delay_ms}ms"],
            check=True, capture_output=True, text=True
        )
        logger.info(f"Latency {delay_ms}ms injected on {interface}")
        ...
        result["output"] = f"Injected {delay_ms}ms latency for {duration_s}s on {interface}"
...
def inject_packet_loss(experiment_id, parameters=None):
    """Inject packet loss using tc (traffic control) or Toxiproxy."""
    ...
    try:
        ...
        logger.info(f"Packet loss {loss_pct}% injected on {interface}")
        time.sleep(duration_s)
        subprocess.run(
            ["tc", "qdisc", "del", "dev", interface, "root"],
            check=True, capture_output=True, text=True
        ...
        result["status"] = "completed"
        result["output"] = f"Injected {loss_pct}% packet loss for {duration_s}s on {interface}"
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def stress_cpu(experiment_id, parameters=None):
    """Stress CPU using stress-ng or /dev/urandom burn."""
    logger.info(f"Starting stress_cpu for experiment {experiment_id}")
    result = {"experiment_id": experiment_id, "status": "running"}
    try:
        params = parameters or {}
        cpu_percent = params.get("cpu_percent", 90)
        duration_s = params.get("duration_s", 60)
        workers = params.get("workers", 4)
        # Using stress-ng if available, else fallback to dd
        try:
            subprocess.run(
                ["stress-ng", "--cpu", str(workers), "--cpu-load", str(cpu_percent),
                 "--timeout", f"{duration_s}s"],
                check=True, capture_output=True, text=True
            )
        except FileNotFoundError:
            # Fallback: burn CPU with /dev/urandom
            for i in range(workers):
                subprocess.Popen(
                    ["dd", "if=/dev/urandom", "of=/dev/null", f"bs=1M"],
                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
                )
            time.sleep(duration_s)
            subprocess.run(["pkill", "-f", "dd if=/dev/urandom"], capture_output=True)
        result["status"] = "completed"
        result["output"] = f"CPU stressed at {cpu_percent}% for {duration_s}s with {workers} workers"
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def stress_memory(experiment_id, parameters=None):
    """Stress memory using stress-ng or mmap allocation."""
    logger.info(f"Starting stress_memory for experiment {experiment_id}")
    result = {"experiment_id": experiment_id, "status": "running"}
    try:
        params = parameters or {}
        mem_mb = params.get("memory_mb", 1024)
        duration_s = params.get("duration_s", 60)
        try:
            subprocess.run(
                ["stress-ng", "--vm", "1", "--vm-bytes", f"{mem_mb}M",
                 "--timeout", f"{duration_s}s"],
                check=True, capture_output=True, text=True
            )
        except FileNotFoundError:
            # Fallback: allocate memory with python
            data = b"0" * (mem_mb * 1024 * 1024)
            time.sleep(duration_s)
            del data
        result["status"] = "completed"
        result["output"] = f"Memory stressed at {mem_mb}MB for {duration_s}s"
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def fill_disk(experiment_id, parameters=None):
    """Fill disk to test storage exhaustion handling."""
    logger.info(f"Starting fill_disk for experiment {experiment_id}")
    result = {"experiment_id": experiment_id, "status": "running"}
    try:
        params = parameters or {}
        fill_mb = params.get("fill_mb", 5000)
        target_path = params.get("target_path", "/tmp/chaos-fill")
        duration_s = params.get("duration_s", 60)
        os.makedirs(target_path, exist_ok=True)
        fill_file = os.path.join(target_path, "chaos-fill.dat")
        # Write in chunks to avoid memory issues
        chunk_size = 1024 * 1024  # 1MB
        written = 0
        with open(fill_file, "wb") as f:
            while written < fill_mb * 1024 * 1024:
                chunk = os.urandom(min(chunk_size, fill_mb * 1024 * 1024 - written))
                f.write(chunk)
                written += len(chunk)
        logger.info(f"Filled {fill_mb}MB at {target_path}")
        time.sleep(duration_s)
        # Clean up
        os.remove(fill_file)
        result["status"] = "completed"
        result["output"] = f"Filled {fill_mb}MB at {target_path} for {duration_s}s"
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def delete_pod(experiment_id, parameters=None):
    """Delete a Kubernetes pod to test self-healing."""
    ...
    try:
        ...
        if label_selector:
            ...
            result["output"] = f"Deleted pods with label {label_selector} in {namespace}"
        else:
            subprocess.run(
                ["kubectl", "delete", "pod", pod_name, "-n", namespace, "--grace-period=0", "--force"],
                check=True, capture_output=True, text=True
            )
            result["output"] = f"Deleted pod {pod_name} in {namespace}"
        result["status"] = "completed"
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def kill_container(experiment_id, parameters=None):
    """Kill a Docker container to test restart policies."""
    ...
    try:
        ...
        result["status"] = "completed"
        result["output"] = f"Killed container {container_name} with {signal}"
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def drain_node(experiment_id, parameters=None):
    """Drain a Kubernetes node to test pod rescheduling."""
    logger.info(f"Starting drain_node for experiment {experiment_id}")
    result = {"experiment_id": experiment_id, "status": "running"}
    try:
        params = parameters or {}
        node_name = params.get("node_name")
        drain_timeout = params.get("drain_timeout", "60s")
        if not node_name:
            raise ValueError("node_name required")
        subprocess.run(
            ["kubectl", "drain", node_name, "--ignore-daemonsets",
             "--delete-emptydir-data", "--force", "--timeout", drain_timeout],
            check=True, capture_output=True, text=True
        )
        result["status"] = "completed"
        result["output"] = f"Drained node {node_name} with timeout {drain_timeout}"
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def fail_dns(experiment_id, parameters=None):
    """Fail DNS resolution to test DNS redundancy."""
    ...
    try:
        ...
        subprocess.run(["cp", "/etc/resolv.conf", "/etc/resolv.conf.bak"],
                       check=True, capture_output=True)
        # Write a broken resolv.conf
        with open("/etc/resolv.conf", "w") as f:
            f.write("nameserver 192.0.2.1\n")  # TEST-NET, will fail
        logger.info(f"DNS failure injected for {target_domain}")
        time.sleep(duration_s)
        # Restore
        subprocess.run(["cp", "/etc/resolv.conf.bak", "/etc/resolv.conf"],
                       check=True, capture_output=True)
        subprocess.run(["rm", "/etc/resolv.conf.bak"], capture_output=True)
        result["status"] = "completed"
        result["output"] = f"DNS failure injected for {duration_s}s"
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def check_error_rate(experiment_id, parameters=None):
    """Check error rate from monitoring system (Prometheus/Grafana)."""
    ...
    try:
        ...
        threshold_pct = params.get("threshold_percent", 5)
        query = f'sum(rate(http_requests_total{{status=~"5..",service="{service}"}}[5m])) / sum(rate(http_requests_total{{service="{service}"}}[5m])) * 100'
        ...
        result["status"] = "completed"
        result["output"] = f"Error rate: {error_rate:.2f}% (threshold: {threshold_pct}%)"
        result["error_rate"] = error_rate
        result["threshold"] = threshold_pct
        result["breach"] = error_rate > threshold_pct
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def check_latency(experiment_id, parameters=None):
    """Check P99 latency from monitoring system."""
    ...
    try:
        ...
        resp = requests.get(f"{prometheus_url}/api/v1/query", params={"query": query}, timeout=10)
        data = resp.json()
        p99_ms = float(data["data"]["result"][0]["value"][1]) if data["data"]["result"] else 0.0
        result["status"] = "completed"
        result["output"] = f"P99 latency: {p99_ms:.0f}ms (threshold: {threshold_ms}ms)"
        result["p99_latency_ms"] = p99_ms
        result["threshold_ms"] = threshold_ms
        result["breach"] = p99_ms > threshold_ms
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def check_circuit_breaker(experiment_id, parameters=None):
    """Check circuit breaker state via service health endpoint."""
    logger.info(f"Starting check_circuit_breaker for experiment {experiment_id}")
    result = {"experiment_id": experiment_id, "status": "running"}
    try:
        params = parameters or {}
        health_url = params.get("health_url", "http://localhost:8080/health")
        expected_state = params.get("expected_state", "CLOSED")
        resp = requests.get(health_url, timeout=5)
        health = resp.json()
        cb_state = health.get("circuit_breaker", health.get("state", "UNKNOWN"))
        result["status"] = "completed"
        result["output"] = f"Circuit breaker: {cb_state} (expected: {expected_state})"
        result["current_state"] = cb_state
        result["expected_state"] = expected_state
        result["match"] = cb_state == expected_state
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def check_queue_depth(experiment_id, parameters=None):
    """Check message queue depth (RabbitMQ/Kafka/SQS)."""
    ...
    try:
        ...
        if queue_type == "rabbitmq":
            mgmt_url = params.get("mgmt_url", "http://localhost:15672")
            resp = requests.get(f"{mgmt_url}/api/queues/{queue_name}", auth=("guest", "guest"), timeout=5)
            depth = resp.json().get("messages", 0)
        elif queue_type == "kafka":
            # Use kafka-consumer-groups CLI
            resp = subprocess.run(
                ["kafka-consumer-groups", "--bootstrap-server", "localhost:9092",
                 "--describe", "--group", queue_name],
                capture_output=True, text=True
            )
            depth = sum(int(line.split()[5]) for line in resp.stdout.strip().split('\n')[1:] if line.strip())
        else:
            depth = 0
        result["status"] = "completed"
        result["output"] = f"Queue depth: {depth} (threshold: {threshold})"
        result["depth"] = depth
        result["threshold"] = threshold
        result["breach"] = depth > threshold
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def rollback_experiment(experiment_id, parameters=None):
    """Rollback a chaos experiment by removing injected faults."""
    logger.info(f"Starting rollback_experiment for experiment {experiment_id}")
    result = {"experiment_id": experiment_id, "status": "running"}
    try:
        params = parameters or {}
        fault_type = params.get("fault_type", "all")
        # Remove tc qdisc rules
        if fault_type in ("latency", "packet_loss", "all"):
            interface = params.get("interface", "eth0")
            subprocess.run(["tc", "qdisc", "del", "dev", interface, "root"],
                          capture_output=True)
        # Restore DNS
        if fault_type in ("dns", "all"):
            if os.path.exists("/etc/resolv.conf.bak"):
                subprocess.run(["cp", "/etc/resolv.conf.bak", "/etc/resolv.conf"], check=True)
                subprocess.run(["rm", "/etc/resolv.conf.bak"], capture_output=True)
        # Kill stress processes
        if fault_type in ("cpu", "memory", "all"):
            subprocess.run(["pkill", "-f", "stress-ng"], capture_output=True)
        result["status"] = "completed"
        result["output"] = f"Rolled back fault type: {fault_type}"
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def notify_stakeholders(experiment_id, parameters=None):
    """Notify stakeholders about experiment status via Slack/email."""
    logger.info(f"Starting notify_stakeholders for experiment {experiment_id}")
    result = {"experiment_id": experiment_id, "status": "running"}
    try:
        params = parameters or {}
        message = params.get("message", f"Chaos experiment {experiment_id} status update")
        channel = params.get("channel", "#chaos-engineering")
        webhook_url = params.get("webhook_url")
        if webhook_url:
            payload = {"channel": channel, "text": message}
            requests.post(webhook_url, json=payload, timeout=10)
            result["output"] = f"Notified {channel} via webhook"
        else:
            logger.info(f"Notification (no webhook): [{channel}] {message}")
            result["output"] = f"Notification logged to [{channel}]: {message[:80]}..."
        result["status"] = "completed"
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def log_experiment_event(experiment_id, parameters=None):
    """Log experiment event to persistent storage."""
    ...
    try:
        ...
        log_dir = params.get("log_dir", "/tmp/chaos-logs")
        os.makedirs(log_dir, exist_ok=True)
        log_file = os.path.join(log_dir, f"{experiment_id}.jsonl")
        with open(log_file, "a") as f:
            f.write(json.dumps(log_entry) + "\n")
        result["status"] = "completed"
        result["output"] = f"Event logged: {event_type}"
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def record_metric(experiment_id, parameters=None):
    """Record experiment metric to Prometheus or local storage."""
    logger.info(f"Starting record_metric for experiment {experiment_id}")
    result = {"experiment_id": experiment_id, "status": "running"}
    try:
        params = parameters or {}
        metric_name = params.get("metric_name", "chaos_experiment_result")
        metric_value = params.get("value", 1)
        labels = params.get("labels", {})
        prometheus_url = params.get("prometheus_url")
        if prometheus_url:
            # Push to Pushgateway
            label_str = ",".join(f'{k}="{v}"' for k, v in labels.items())
            payload = f'{metric_name}{{{label_str}}} {metric_value}\n'
            requests.post(f"{prometheus_url}/metrics/job/chaos_experimenter/instance/{experiment_id}",
                         data=payload, timeout=10)
            result["output"] = f"Metric {metric_name}={metric_value} pushed to Prometheus"
        else:
            logger.info(f"Metric: {metric_name}={metric_value} labels={labels}")
            result["output"] = f"Metric logged: {metric_name}={metric_value}"
        result["status"] = "completed"
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def analyze_results(experiment_id, parameters=None):
    """Analyze experiment results against steady-state hypothesis."""
    ...
    try:
        ...
        breach_count = sum(1 for e in events if e.get("details", {}).get("breach", False))
        total_checks = sum(1 for e in events if e.get("event_type") == "check")
        hypothesis_validated = breach_count == 0
        result["status"] = "completed"
        result["output"] = f"Analysis: {breach_count}/{total_checks} checks breached. Hypothesis: {'VALIDATED' if hypothesis_validated else 'VIOLATED'}"
...
def calculate_blast_radius(experiment_id, parameters=None):
    """Calculate blast radius based on affected services and dependencies."""
    logger.info(f"Starting calculate_blast_radius for experiment {experiment_id}")
    result = {"experiment_id": experiment_id, "status": "running"}
    try:
        params = parameters or {}
        target_service = params.get("service", "unknown")
        dependency_map = params.get("dependencies", {})
        affected = set()
        ...
        while queue:
            ...
            if svc in affected:
                continue
            affected.add(svc)
            queue.extend(dependency_map.get(svc, []))
        radius_score = len(affected)
        radius_level = "LOW" if radius_score <= 2 else "MEDIUM" if radius_score <= 5 else "HIGH"
        result["status"] = "completed"
        result["output"] = f"Blast radius: {radius_level} ({radius_score} services affected)"
        result["radius_score"] = radius_score
        result["radius_level"] = radius_level
...
def validate_hypothesis(experiment_id, parameters=None):
    """Validate the steady-state hypothesis against collected metrics."""
    logger.info(f"Starting validate_hypothesis for experiment {experiment_id}")
    result = {"experiment_id": experiment_id, "status": "running"}
    try:
        params = parameters or {}
        hypothesis = params.get("hypothesis", "System maintains steady state under fault")
        ...
        result["status"] = "completed"
        result["output"] = f"Hypothesis: {'VALIDATED' if validated else 'VIOLATED'} — {hypothesis}"
...
def generate_report(experiment_id, parameters=None):
    """Generate experiment report with findings and recommendations."""
    logger.info(f"Starting generate_report for experiment {experiment_id}")
    result = {"experiment_id": experiment_id, "status": "running"}
    try:
        params = parameters or {}
        experiment_name = params.get("name", experiment_id)
        hypothesis = params.get("hypothesis", "Unknown")
        ...
        report_dir = params.get("report_dir", "/tmp/chaos-reports")
        os.makedirs(report_dir, exist_ok=True)
        report_file = os.path.join(report_dir, f"{experiment_id}-report.json")
        with open(report_file, "w") as f:
            json.dump(report, f, indent=2)
        result["status"] = "completed"
        result["output"] = f"Report generated: {report_file}"
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def track_remediation(experiment_id, parameters=None):
    """Track remediation actions for experiment findings."""
    logger.info(f"Starting track_remediation for experiment {experiment_id}")
    result = {"experiment_id": experiment_id, "status": "running"}
    try:
        params = parameters or {}
        finding_id = params.get("finding_id")
        action = params.get("action", "investigating")
        owner = params.get("owner", "unassigned")
        status = params.get("status", "open")
        remediation_file = params.get("remediation_file", "/tmp/chaos-remediation.json")
        remediations = []
        ...
        result["status"] = "completed"
        result["output"] = f"Remediation tracked: {action} (owner: {owner}, status: {status})"
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def schedule_game_day(experiment_id, parameters=None):
    ...
    try:
        ...
        gameday_dir = params.get("gameday_dir", "/tmp/chaos-gamedays")
        os.makedirs(gameday_dir, exist_ok=True)
        gameday_file = os.path.join(gameday_dir, f"{experiment_id}.json")
        with open(gameday_file, "w") as f:
            json.dump(gameday, f, indent=2)
        result["status"] = "completed"
        result["output"] = f"GameDay scheduled: {title} on {date} with {len(participants)} participants"
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
...
def check_steady_state(experiment_id, parameters=None):
    """Verify system is in steady state before/during/after experiment."""
    logger.info(f"Starting check_steady_state for experiment {experiment_id}")
    result = {"experiment_id": experiment_id, "status": "running"}
    try:
        params = parameters or {}
        prometheus_url = params.get("prometheus_url", "http://localhost:9090")
        indicators = params.get("indicators", ["error_rate", "latency_p99", "availability"])
        steady = True
        details = {}
        for indicator in indicators:
            query = params.get(f"query_{indicator}", f'up{{job="{indicator}"}}')
            try:
        ...
        result["status"] = "completed"
        result["output"] = f"Steady state: {'MAINTAINED' if steady else 'DISRUPTED'} — indicators: {details}"
        result["steady"] = steady
        result["indicators"] = details
    except Exception as exc:
        result["status"] = "failed"
        result["error"] = str(exc)
